...
$modulePath = "..\Scripts"
$logPath = "..\Logs"
$monitorSettingsPath = ".\$($title).csv"  #config file with files/folders to monitor
...
#get email settings
$emailSender = Get-Content  ".\emailMonitorSender.txt"
$emailRecipients = Get-Content ".\emailMonitorRecipients.txt"
...
{   
    ...
    if  ($WithinStartStopTime)
    {
        ...
        {
            $alert = "$($monitor.Title):`t`r`nFolder:`t$($monitor.Folder)`t`r`nFile not found:`t$($filter)`t`r`n"
            $fileAlerts += $alert
