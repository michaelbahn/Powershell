...
$events = @()
 $winEvents = Get-WinEvent -ComputerName $server  -FilterHashtable @{ LogName='Security'; StartTime=$date; Id='4624' } | Select-Object $properties 

if ($winEvents -ne $null)
{
    ...
    $winEvents | ConvertTo-Html  -Head $htmlHeader | Out-File $tempFile
