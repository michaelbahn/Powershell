    ...
    foreach ($errorFile in $errorFiles)
    {
        ...
        elseif ($archiveBatchPath -ne $archiveBatchPathError)
        {
            ...
            if (SendToFixer $tifCount $jpegCount $pdfCount)
            {
				...
				#first check input folder if any old files there.
                $inputFolder = Get-Item  "$($ddfFixerInputPath)\*.*"  
                if ($inputFolder.Length -gt 0)
                {
                    Send-MailMessage -SmtpServer smtp.edd.ca.gov -From $sender -To $admins -Subject "RBE Fixer Input Folder contains files" -Body $inputFolder
                }

                #move batch to input folder 
				Move-Item $batchPath -destination $ddfFixerInputPath  -Force 
