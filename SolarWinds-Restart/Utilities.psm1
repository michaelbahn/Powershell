...
function Initialize-CSV ($logPath, $logFileName)
{
    If (!(test-path $logPath))   #create log folder if it doesn't exist
    {
          New-Item -ItemType Directory -Force -Path $logPath
    ...
    $logFileNew = New-Item $logFilePath -ItemType File
     return $logFileNew
}

#function to write to csv
function Write-CSV($logFileObject, [string] $logText)
{
        try
        {
...
function Get-ContentWithComments ($file)
{
    #verify file exists
    if (!(Test-Path $file ))
    {
    ...
    $rows = Get-Content $file

    foreach ($row in $rows) 
    {   
        if ($row.Substring(0,1) -ne "#")
        {
            $returnList += $row
        }
    }
...
function Convert-FilePathToHTML ($filePath)
{
    $html = '<a href="'
    $html += "$($filePath.FullName)"
    $html += '">'
    $html += "$($filePath.Name)</a>"
    return $html  
}
