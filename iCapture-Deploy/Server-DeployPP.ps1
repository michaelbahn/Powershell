...
#read settings
$deployPath = "\\dgvmicapimgpp01\d$\Deployment\FORMDB_DE5617_Updates"
$deployPathLength = $deployPath.Length
$rollbackSuffix = "_05092019"
...
#get list of servers
$servers = Get-Content  ("servers-PP.txt")

foreach ($server in $servers) 
{   
    $deployPathServerFiles = "$($deployPath)"
     ...
     if ((Test-Path $deployPathServerFiles))
    {
         ...
         if ((Test-Path $deployServerFormdbFiles))
        {
            ...
            foreach ($newFile in $newFiles)
            {
                ...
                $destinationPath = "\\$($server)$($sourcePath.Substring($deployPathLength))"
