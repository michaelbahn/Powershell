...
$workingFolder = "D:\Powershell-Production-scripts\SIDES\Temp"
...
{  
 ...
 Add-Content $htmlFileName "<td width='40%' align='center'><strong>Error</strong></td>" 
 Add-Content $htmlFileName "<td width='40%' align='center'><strong>Fix</strong></td>" 
 Add-Content $htmlFileName "</tr>" 
 }  
...
function ConvertCsvToPDF ($csvFile, [ref] $pdfFileName)
{
    ...
    Write-Log $logFile "Opening $($csvFile)"
    ...
    Write-Log $logFile "Saving as  $($pdfFileName.Value)"
    $workbook.Saved = $true
    ...
    Write-Log $logFile "ConvertToPDF completed: $($pdfFileName.Value)"
    return $pdfFileName
}
