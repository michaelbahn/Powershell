...
#get all batches fixed by Fix-DDF
    $batchFoldersDDF = Get-ChildItem -Path $ddfFixFolderPath -Directory -ErrorAction SilentlyContinue | Where-Object LastWriteTime -ge $todayMidnight
...
#get all batches fixed by Fix-RBE   
    $batchFoldersRBE = Get-ChildItem -Path $rbeFixFolderPath -Directory -ErrorAction SilentlyContinue | Where-Object LastWriteTime -ge $todayMidnight
